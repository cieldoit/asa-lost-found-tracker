/* ============================================================
   STAFF HEADER WEB COMPONENT
============================================================ */
class StaffHeader extends HTMLElement {
  connectedCallback() {
    this.render();
    this.initListeners();
  }
          ...
          <div class="header-right">
            ...
            <div class="profile-wrap">
              <button class="profile-btn" id="headerProfileBtn">
                ...
                <span class="profile-name" id="headerProfileName">Staff</span>
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="14" height="14"><polyline points="6 9 12 15 18 9"/></svg>
              </button>
              <div class="dropdown-content" id="headerProfileDropdown">
                <div class="user-info">
                  <div class="pd-name" id="headerDropName">Staff</div>
  ...
  initListeners() {
    ...
    seeAllBtn?.addEventListener('click', e => {
      e.preventDefault();
      e.stopPropagation();
      if (document.getElementById('staffNotificationsModal')) openStaffNotificationsModal();
      notifDropdown.classList.remove('show');
    });
  ...
  renderNotifications(notifs) {
    window.staffNotifications = Array.isArray(notifs) ? notifs : [];
    const list = this.querySelector('#headerNotifList');
    const dot  = this.querySelector('#headerNotifDot');
    if (list) list.onclick = event => { const item = event.target.closest('[data-notif-id]'); if (item) staffMarkNotificationRead(item.dataset.notifId); };
    if (!list) return;

    if (!window.staffNotifications.length) {
      list.innerHTML = `<div style="padding:20px;text-align:center;color:#9ca3af;font-size:13px">No notifications yet.</div>`;
      if (dot) dot.classList.add('hidden');
      return;
    }

    const unread = window.staffNotifications.filter(isUnreadNotification);
    if (dot) { dot.textContent = unread.length; dot.classList.toggle('hidden', unread.length === 0); }

    list.innerHTML = window.staffNotifications.slice(0, 5).map(n => `
...
customElements.define('staff-header', StaffHeader);
...
function openStaffNotificationsModal() {
  document.getElementById('staffNotificationsModal')?.remove();
  const notifs = window.staffNotifications || [];
  const modal = document.createElement('div');
  modal.id = 'staffNotificationsModal';
  ...
  modal.innerHTML = `
    <div style="width:min(560px,100%);max-height:82vh;background:#fff;border-radius:16px;box-shadow:0 24px 70px rgba(0,0,0,.25);overflow:hidden;">
      <div style="display:flex;align-items:center;justify-content:space-between;padding:18px 20px;border-bottom:1px solid #e5e7eb;">
        ...
        <button type="button" onclick="closeStaffNotificationsModal()" style="border:0;background:transparent;font-size:24px;line-height:1;cursor:pointer;color:#6b7280;">&times;</button>
      </div>
      <div style="max-height:66vh;overflow:auto;">
        ${notifs.length ? notifs.map(n => `
          <button type="button" onclick="staffMarkNotificationRead('${n.notifID}')" style="width:100%;display:flex;gap:12px;text-align:left;padding:15px 18px;border:0;border-bottom:1px solid #e5e7eb;background:${isUnreadNotification(n) ? '#f0fdf4' : '#fff'};cursor:pointer;">
  ...
  modal.addEventListener('click', e => {
    if (e.target === modal) closeStaffNotificationsModal();
  });
  document.body.appendChild(modal);
}

function closeStaffNotificationsModal() {
  document.getElementById('staffNotificationsModal')?.remove();
}

async function staffMarkNotificationRead(notifID) {
  if (!notifID) return;
  try {
    await NotifAPI.markRead(notifID);
    await loadNotifications();
    openStaffNotificationsModal();
  } catch (err) {
    console.warn('Could not mark notification read:', err.message);
  }
}

window.closeStaffNotificationsModal = closeStaffNotificationsModal;
window.staffMarkNotificationRead = staffMarkNotificationRead;
/* ============================================================
   HELPER: close all dropdowns
============================================================ */
function closeAllDropdowns() {
  const hdr = document.querySelector('staff-header');
  if (!hdr) return;
...
/* ============================================================
   IMAGE PREVIEW (only used for lost items now)
============================================================ */
...
function removePreview(areaId, inputId) {
  ...
  const input = document.getElementById(inputId);
  if (area) {
    // Generic onclick that triggers the file input click
...
async function submitFoundItem() {
  ...
  const submitBtn = document.querySelector('#page-report-found .btn-submit, #page-report-found button[onclick*="submitFoundItem"]');
  if (submitBtn) { submitBtn.disabled = true; submitBtn.textContent = 'Submitting...'; }
...
async function syncCurrentUserProfile() {
  const storedName = typeof Auth !== 'undefined' ? Auth.getUser() : (localStorage.getItem('userName') || 'Staff');
  const storedRole = typeof Auth !== 'undefined' ? Auth.getRole() : (localStorage.getItem('role') || 'Staff');
  ...
  const displayName = user.userName || storedName || 'Staff';
  ...
  document.querySelector('staff-header')?.setUsername?.(displayName, user.role || storedRole);
...
async function submitAccountInfo(e) {
  ...
  if (!first || !last) {
    showToast('error', 'Incomplete Form', 'Please fill in your name.');
    return;
  }
  ...
  try {
    ...
    document.querySelector('staff-header')?.setUsername(userName);
    localStorage.setItem('userName', userName);
    localStorage.setItem('asa_user', userName);
    populateAccountForm({ userName, email });

    showToast('success', 'Account Updated!', 'Your profile has been saved successfully.');
  } catch (err) {
...
async function submitPasswordChange(e) {
  ...
  try {
    ...
    showToast('error', 'Error', err.message || 'Could not change password.');
  }
}
...
function getCategoryEmoji(cat) {
  ...
  const map = {
    'electronics': '<i class="fa-solid fa-mobile-screen-button" aria-hidden="true"></i>', 'clothing': '<i class="fa-solid fa-shirt" aria-hidden="true"></i>', 'accessories': '<i class="fa-solid fa-glasses" aria-hidden="true"></i>',
    'documents': '<i class="fa-solid fa-file-lines" aria-hidden="true"></i>', 'books': '<i class="fa-solid fa-book" aria-hidden="true"></i>', 'keys': '<i class="fa-solid fa-key" aria-hidden="true"></i>', 'others': '<i class="fa-solid fa-box" aria-hidden="true"></i>'
  };
...
function renderItemGrid(gridId, items) {
  ...
  grid.innerHTML = '';
  if (!items.length) {
    grid.innerHTML = '<p style="text-align:center;color:#9ca3af;padding:40px 0">No items found.</p>';
    return;
  }
...
async function loadNotifications() {
  try {
    ...
    document.querySelector('staff-header')?.renderNotifications(notifs);
  } catch (err) {
